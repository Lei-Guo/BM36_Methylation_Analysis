...
Rscript -e "rmarkdown::render('correlation_analysis_probe_ad_traits.Rmd')"
...
# only keep desired AD traits
mVal_age_adj_meta <- mVal_age_adj_meta[, c("CDR", "PlaqueMean", "bbscore", "CERAD")]
```


Correlation analysis between probes and AD traits
```{r Correlation analysis between probes and AD traits}
# transpose mvalue matrix
mVal_age_adj_mx_t <- t(mVal_age_adj_mx)

# convert CERAD to numerical
map_cerad <- setNames(seq(4), c("NL", "possAD", "probAD", "defAD"))
mVal_age_adj_meta$CERAD <- map_cerad[mVal_age_adj_meta$CERAD]

# correlation test
cor_results <- as.data.frame(corRhoPvalSpearmanPearson_TwoMatrices(mVal_age_adj_mx_t, mVal_age_adj_meta))
...
Writing the results
```{r}
fwrite(cor_results, "/sc/orga/projects/zhangb03a/lei_guo/BM36_methylation_analysis/correlation_Results/methy_probe_ad_trait_cor_results.csv")
```
